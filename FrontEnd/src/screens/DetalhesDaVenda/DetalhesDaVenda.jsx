...
import Amortizar from "./components/Amortizar/Amortizar";
...
function DetalhesDaVenda() {
  ...
  const [editarAmortizar, setEditarAmortizar] = useState(false);
  ...
  const carregarDados = useCallback(async () => {
    ...
    try {
      ...
      const [clienteResponse, produtosResponse, pagamentosResponse] =
        await Promise.all([
      ...
      if (vendaData.status !== STATUS_VENDA.CONCLUIDA) {
        const parcelas = await vendaFetch.listarVendasCrediarioVenda(id);
        parcelasData = parcelas.filter((parcela) => parcela.status !== "pago");
      }
  ...
  const carnePagamentoDados = useMemo(
    () => ({
      ...
      cliente: {
        ...
        cpf_cnpj: services.formatarCPF(
          dadosVenda.cliente?.cpf_cnpj || "indefinido"
        ),
      },
  ...
  const handleEnviarWhatsApp = useCallback(async () => {
    ...
    const tipoCompra =
      dadosVenda.venda.status === STATUS_VENDA.ORCAMENTO
        ? "orçamento"
        : "venda";
    ...
    const dados = {
      ...
      mensagem: {
        ...
        valor_total_compra: services.formatarCurrency(
          dadosVenda.venda.valor_total
        ),
      },
    };
  ...
  const handleToggleEditar = useCallback(() => {
    setEditarAmortizar((prev) => !prev);
  }, []);
  ...
  // Handler para amortizar crediário
  const handleAmortizarCrediario = useCallback(() => {
    setModalAtiva("amortizar");
  }, [modalAtiva]);
        ...
        <div>
          ...
          <p>
            ...
            {services.formatarNumeroCelular(
              dadosVenda.cliente?.telefone || "indefinido"
            )}
          </p>
          <p>
            <strong>CPF: </strong>
    ...
    () =>
      dadosVenda.pagamentos.map((pagamento, index) => (
        <div key={pagamento.id || index}>
          {services.formatarCurrency(pagamento.valor)} :{" "}
  ...
  const MenuNotas = useMemo(
    ...
    [
      escolherNotas,
      dadosVenda.venda.status,
      handleDownloadNotaGrande,
      handleDownloadCarne,
    ]
  );

  // Menu de amortização memoizado
  const MenuAmortizar = useMemo(
    () => (
      <div
        id="EscolherAmortizarArticle"
        style={{
          position: "absolute",
          bottom: "100%",
          left: 0,
          ...
          zIndex: 10,
          overflow: "hidden",
          transform: editarAmortizar ? "translateY(0)" : "translateY(10px)",
          opacity: editarAmortizar ? 1 : 0,
          pointerEvents: editarAmortizar ? "auto" : "none",
          transition: "all 0.3s ease",
        }}
      >
        {dadosVenda.pagamentos?.[0]?.tipo_pagamento === "crediário próprio" && (
          <p
            onClick={handleAmortizarCrediario}
            style={{ cursor: "pointer", padding: "8px", margin: 0 }}
          >
            Amortizar crediário
          </p>
    ...
    [editarAmortizar, handleAmortizarCrediario]
  );
  ...
  const renderModal = () => {
    switch (modalAtiva) {
      case "amortizar":
        return <Amortizar fechar={setModalAtiva} dados={dadosVenda.parcelas} />;
      case null:
        return null;
    }
  };
      ...
      <div id="DetalhesDaVendaDisplay">
        ...
        <div id="DetalhesDaVendaDisplay2">
          ...
          <div id="DetalhesDaVendaDisplay2Pt3">
            ...
            <div id="areaButtonsDetalhesVenda" style={{ position: "relative" }}>
              {MenuAmortizar}

              <button
                className="ButãoEditarDetalhesDaVenda ButãoDetalhesDaVenda"
